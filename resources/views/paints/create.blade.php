        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-semibold mb-6">Add New Paint</h3>
                    
                    <form method="POST" action="{{ route('paints.store') }}" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex">
                                ...
                                <div id="color_preview" class="w-12 border border-l-0 border-gray-300 rounded-r-md"
                                     style="background-color: {{ old('hex_color', '#ffffff') }}"></div>
                            </div>
                            @error('hex_color')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="form" id="form" required
                                    ...
                                    <option value="spray" {{ old('form') == 'spray' ? 'selected' : '' }}>Spray Paint</option>
                                    <option value="emulsion" {{ old('form') == 'emulsion' ? 'selected' : '' }}>Emulsion</option>
                                    <option value="acrylic" {{ old('form') == 'acrylic' ? 'selected' : '' }}>Acrylic</option>
                                    <option value="gloss" {{ old('form') == 'gloss' ? 'selected' : '' }}>Gloss</option>
                                    <option value="matt" {{ old('form') == 'matt' ? 'selected' : '' }}>Matt</option>
                                    <option value="satin" {{ old('form') == 'satin' ? 'selected' : '' }}>Satin</option>
                                </select>
                                @error('form')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div>
                            ...
                            <textarea name="color_description" id="color_description" rows="3"
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                      placeholder="A vibrant red that captures the warmth of a summer sunset...">{{ old('color_description') }}</textarea>
